...
export default function BlogFullRightSidebarWithFrame(props) {
    const userSettings = props?.uiSettings;
    return (
        <div className='container p-0'>
            <section className='main'>
                <div className='row' style={{ padding: '60px 0px 40px 0px' }}>
                    <div className="col-12 col-md-8 pb-5 order-1">
                        {cardSet?.map((item, index) => {
                            return (<BlogImageCardPostBlk setData={item} key={index} />)
                        })}
                    </div>
...
BlogFullRightSidebarWithFrame.getLayout = function getLayout(page) {
